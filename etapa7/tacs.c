...
TAC* makeWhenThen(TAC* code0, TAC* code1) {
    ...
    iftac = tacCreate(TAC_IF, newlabel,code0?code0->res:0,0);
        ...
        switch(tac->type) {
            ...
            case TAC_GT:
                ...
                fprintf(fout, 
                    ...
                    "\tjmp FIMHUE%d\n"
                    "HUE%d:\n\tmovl $1, _%s(%%rip)\n"
                    "FIMHUE%d:\n ", tac->res->text,tac->op1->text,tac->op2->text,j,tac->res->text,j,j,tac->res->text,j);
                j++;
                break;
                break;
            case TAC_LT:
...
TAC* tacOptimization(TAC *tac){
	...
	for(tacAux = tac; tacAux; tacAux = tacAux->next){
		switch(tacAux->type){
			case TAC_SYMBOL:
				if(!gadFind(tacAux->res, gadAux)){
					newGAD = gadCreate(tacAux, gadAux, nodeNumber++);
					if(gadAux){
						gadAux->next = newGAD;
						newGAD->prev = gadAux;
					}
...
void gadPrint(GAD *gad, int level){
	int i;
	fprintf(stderr, "\n");
	for(i = 0; i < level; i++)
		fprintf(stderr, "   ");
	...
	{ 
		//fprintf(stderr, "\n(%d, ", gad->type);
		if(gad->symbols[i])
			fprintf(stderr, ", %s", gad->symbols[i]->text);
		/*else
			fprintf(stderr, "0, ");
		if(gad->op1) 
			fprintf(stderr, "%s, ", gad->op1->text);
		else
			fprintf(stderr, "0, ");
		if(gad->op2) 
			fprintf(stderr, "%s)", gad->op2->text);
		else
			fprintf(stderr, "0)");*/
	}
	fprintf(stderr, ")");
	if(gad->son1)
		gadPrint(gad->son1, level + 1);
	if(gad->son2)
		gadPrint(gad->son2, level + 1);
